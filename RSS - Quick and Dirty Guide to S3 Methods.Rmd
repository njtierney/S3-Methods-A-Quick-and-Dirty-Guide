...
S3 methods are a way of writing functions in R that do different things for objects with different classes. For example, the function `summary()` used on a dataframe shows descriptive statistics for each variable. For example, the `iris` dataset.
...
From what I've read, S3 methods are called such because they came along with the third version of the S Programming Language - the language that R was based upon. Hence, S3.

This guide is by no means complete, it could even be hella inaccurate, and I've probably offended some programmers out there by writing it, but hey - it's the internet, if you think I'm wrong, or I could do this better, leave a comment below, or better yet, **check it out on GitHub**, fork it, change it, and send a pull request.
